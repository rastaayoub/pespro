...
$total_pages = $db->QueryGetNumRows("SELECT id FROM `facebook`".$s_value);
include('../system/libs/apaginate.php');

$mesaj = '';
if(isset($_GET['edit'])){
	$edit = $db->EscapeString($_GET['edit']);
	$pData = $db->QueryFetchArray("SELECT user,url,title,cpc,active FROM `facebook` WHERE `id`='".$edit."'");
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $as = $db->QueryFetchArrayAll("SELECT * FROM `facebook`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
  foreach($as as $sites){
