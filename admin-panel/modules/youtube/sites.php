...
$sql = $db->Query("SELECT id FROM `youtube`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,cpc,active FROM `youtube` WHERE `id`='".$edit."'"));
	$user = $sql['user'];
	$sit = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
	...
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `youtube` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `youtube` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `youtube`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="http://www.youtube.com/watch?v=<?=$sites['url']?>" target="_blank">http://www.youtube.com/watch?v=<?=$sites['url']?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
