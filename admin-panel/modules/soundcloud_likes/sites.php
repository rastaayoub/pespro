...
$sql = $db->Query("SELECT id FROM `soundcloud_likes`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,cpc,active FROM `soundcloud_likes` WHERE `id`='".$edit."'"));
	$user = $sql['user'];
	$sit = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
	...
	$db->Query("UPDATE `soundcloud_likes` SET `user`='".$db->EscapeString($_POST['user'])."', `url`='".$db->EscapeString($_POST['url'])."', `title`='".$db->EscapeString($_POST['title'])."', `cpc`='".$db->EscapeString($_POST['cpc'])."', `active`='".$db->EscapeString($_POST['status'])."' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `soundcloud_likes` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `soundcloud_likes`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
