...
if(isset($_GET['sp'])){
	...
	if($_GET['s_type'] == 1){
		$s_value = ($search != '' ?  " WHERE `title` LIKE '%".$search."%'" : "");
...
$sql = $db->Query("SELECT id FROM `ysub`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,title,cpc,active FROM `ysub` WHERE `id`='".$edit."'"));
	$user = $sql['user'];
	$sit = $sql['title'];
	$status = $sql['active'];
	$cpc = $sql['cpc'];
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `ysub` SET `user`='{$_POST['user']}', `title`='{$_POST['url']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = "<div class=\"message success\"><h3>Success!</h3><p>Site successfully edited!</p></div>";
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `ysub` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `ysub`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="http://www.youtube.com/<?=$sites['title']?>" target="_blank">http://www.youtube.com/<?=$sites['title']?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
