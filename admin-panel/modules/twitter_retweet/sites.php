...
$sql = $db->Query("SELECT id FROM `twitter_retweet`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,tweet_id,cpc,active FROM `twitter_retweet` WHERE `id`='".$edit."'"));

	if(isset($_POST['submit'])){
		$db->Query("UPDATE `twitter_retweet` SET `user`='".$db->EscapeString($_POST['user'])."', `url`='".$db->EscapeString($_POST['url'])."', `tweet_id`='".$db->EscapeString($_POST['tweet_id'])."', `cpc`='".$db->EscapeString($_POST['cpc'])."', `active`='".$db->EscapeString($_POST['status'])."' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `twitter_retweet` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `twitter_retweet`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
