...
$total_pages = $db->QueryGetNumRows("SELECT id FROM `fb_event`".$s_value);
include('../system/libs/apaginate.php');

$mesaj = '';
if(isset($_GET['edit'])){
	$edit = $db->EscapeString($_GET['edit']);
	$sql = $db->QueryFetchArray("SELECT user,url,title,cpc,active FROM `fb_event` WHERE `id`='".$edit."'");
	$user = $sql['user'];
	$sit = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
	...
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `fb_event` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Site successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `fb_event` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $as = $db->QueryFetchArrayAll("SELECT * FROM `fb_event`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
  foreach($as as $sites){
...
?>	
						<tr>
							...
							<td><a href="https://www.facebook.com/events/<?=$sites['url']?>" target="_blank">https://www.facebook.com/events/<?=truncate($sites['url'], 60)?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
