...
$sql = $db->Query("SELECT id FROM `linkedin`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,cpc,active FROM `linkedin` WHERE `id`='".$edit."'"));
	$user = $sql['user'];
	$site = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
	...
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `linkedin` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `linkedin` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
			...
			<div class="header">
				...
				<div class="content">
					...
					<div class="row">
						...
						<div><input type="text" name="url" value="<?=(isset($_POST['url']) ? $_POST['url'] : $site)?>" required="required" /></div>
					</div>
					<div class="row">
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `linkedin`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
