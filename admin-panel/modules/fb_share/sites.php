...
$total_pages = $db->QueryGetNumRows("SELECT id FROM `fb_share`".$s_value);
include('../system/libs/apaginate.php');

$mesaj = '';
if(isset($_GET['edit'])){
	$edit = $db->EscapeString($_GET['edit']);
	$sql = $db->QueryFetchArray("SELECT user,url,title,cpc,active FROM `fb_share` WHERE `id`='".$edit."'");
	$user = $sql['user'];
	$sit = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $as = $db->QueryFetchArrayAll("SELECT * FROM `fb_share`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
  foreach($as as $sites){
