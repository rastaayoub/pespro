...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,p_av,cpc,active FROM `instagram` WHERE `id`='".$edit."'"));
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `instagram` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `p_av`='{$_POST['p_av']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `instagram`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="http://instagram.com/<?=$sites['url']?>" target="_blank">http://instagram.com/<?=truncate($sites['url'], 60)?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
