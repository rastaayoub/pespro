...
$coins = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `transactions` WHERE `type`='0'");
$vip = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `transactions` WHERE `type`='1'");
$reports = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `reports` WHERE `status`='0'");
$creports = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `reports` WHERE `status`!='0'");
$waiting_sales = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `transactions` WHERE `paid`='0'");
...
<section id="content" class="container_12 clearfix" data-sort=true>
	<ul class="stats not-on-phone">
		...
		<li>
			<strong><?=number_format($coins['total']+$vip['total'])?></strong>
			<small>Total Sales</small>
			<span <?=($waiting_sales['total'] > 0 ? 'class="green" ' : '')?>style="margin:4px 0 -10px 0"><?=$waiting_sales['total']?> waiting</span>
		</li>
		<li>
	...
	<div class="alert note" id="version_alert" style="display:none"><a href="https://mn-shop.net/account/download" target="_blank"><span class="icon"></span><strong>There is a new version of this script available for download! Download latest version from MN-Shop.net!</strong></a></div>
	...
	<div class="grid_12">
		<div class="box">
			...
			<div class="content">
				<div class="spacer"></div>
				<div class="full-stats">
					<div class="stat hlist" data-list='[{"val":<?=$reports['total'].','.($reports['total'] > 999 ? '"format":"0,0",' : '')?>"title":"Waiting"},{"val":<?=$creports['total'].','.($creports['total'] > 999 ? '"format":"0,0",' : '')?>"title":"Checked","color":"green"}]' data-flexiwidth=true></div>
				</div>
			</div>
		</div>
	</div>
