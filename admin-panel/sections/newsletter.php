...
$all_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE `activate`='0' AND `banned`!='1'");
$vip_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `banned`!='1') AND `premium`>'0'");
$coins_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `banned`!='1') AND `coins`<'10'");
$active_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `banned`!='1') AND (".time()."-UNIX_TIMESTAMP(`online`)) < 604800");
$inactive_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `banned`!='1') AND (".time()."-UNIX_TIMESTAMP(`online`)) > 604800");
$subscribed_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `banned`!='1') AND `newsletter`='1'");
?>
