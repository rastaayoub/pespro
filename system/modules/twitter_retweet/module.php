...
<h2 class="title"><?=$lang['b_162']?> - Twitter Retweet</h2>
<?
$dbt_value = '';
...
$sites = $db->QueryFetchArrayAll("SELECT a.id, a.url, a.tweet_id, a.cpc, b.premium FROM twitter_retweet a LEFT JOIN users b ON b.id = a.user LEFT JOIN twitter_retweeted c ON c.user_id = '".$data['id']."' AND c.site_id = a.id WHERE a.active = '0' AND (a.max_clicks > a.clicks OR a.max_clicks = '0') AND (a.daily_clicks > a.today_clicks OR a.daily_clicks = '0') AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site_id IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC".($site['mysql_random'] == 1 ? ', RAND()' : '')." LIMIT 14");
...
<div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
	<center>
		...
		<a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=hideref('https://twitter.com/intent/retweet?tweet_id='.$sit['tweet_id'])?>','Retweet','<?=($sit['cpc']-1)?>','1');" class="followbutton"><?=$lang['twitter_06']?></a>
		...
		<span style="position:absolute;bottom:1px;right:2px;"><a href="javascript:void(0);" onclick="report_page('<?=$sit['id']?>','<?=base64_encode($sit['url'])?>','twitter_retweet');"><img src="img/report.png" alt="Report" title="Report" border="0" /></a></span>
	</center>
</div>
<?}?>
</div>
