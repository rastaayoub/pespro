...
if(isset($_POST['get']) && $_POST['pid'] > 0 && !empty($data['id'])){
	...
	$msg = ($result ? '<div class="msg"><div class="info">'.$lang['ysub_14'].'</div></div>' : '<div class="msg"><div class="error">We cannot contact Youtube...</div></div>');
	...
	if(empty($yuser['uid']) || empty($yuser['id']) || empty($data['id']) || $yuser['coins'] < $yuser['cpc'] || $yuser['cpc'] < 2){
		...
		if($db->QueryGetNumRows("SELECT site_id FROM `ysubed` WHERE `site_id`='".$id."' AND `user_id`='".$data['id']."' LIMIT 1") == 0){
			...
			if($valid){
				...
				$db->Query("UPDATE `module_session` SET `ses_key`='".$ses_key."' WHERE (`page_id`='".$yuser['id']."' AND `module`='ysub') AND `ses_key`='".($ses_key-1)."'");
				...
				$msg = '<div class="msg"><div class="success">'.lang_rep($lang['b_358'], array('-NUM-' => ($yuser['cpc']-1))).'</div></div>';
				$type = 'success';
			}else{
				$msg = '<div class="msg"><div class="error">'.$lang['ysub_17'].'</div></div>';
				$type = 'error';
			}
		}else{
			$msg = '<div class="msg"><div class="error">'.$lang['b_300'].'</div></div>';
