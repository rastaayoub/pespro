...
<h2 class="title"><?=$lang['b_162']?> - Twitter Followers</h2>
<?
$dbt_value = '';
...
<div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
	<center>
		<img src="<?=(empty($sit['t_img']) ? 'img/icons/tweet.png' : $sit['t_img'])?>" border="0" alt="@<?=$sit['url']?>" title="@<?=$sit['url']?>" width="48" height="48" class="follower"><br><b><?=$lang['b_42']?></b>: <?=($sit['cpc']-1)?><br>
		<a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=hideref('https://twitter.com/intent/user?screen_name='.$sit['url'])?>','Twitter','<?=($sit['cpc']-1)?>','1');" class="followbutton"><?=$lang['twt_12']?></a>
