<h2 class="title"><?=$lang['b_162']?> - Ask.fm Likes</h2>
<?php
    $dbt_value = '';
    ...
    $sites = $db->QueryFetchArrayAll("SELECT a.id, a.url, a.title, a.img, a.cpc, b.premium FROM askfm_like a LEFT JOIN users b ON b.id = a.user LEFT JOIN askfm_liked c ON c.user_id = '".$data['id']."' AND c.site_id = a.id WHERE a.active = '0' AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site_id IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC".($site['mysql_random'] == 1 ? ', RAND()' : '')." LIMIT 14");
        ...
        <?
            ...
            <script type="text/javascript">
                $(document).ready(function(){  
                        document.getElementById('preview<?=$sit['id']?>').onerror = function() { 
                            document.getElementById('preview<?=$sit['id']?>').src = "img/askfm-default.png"; 
                        }     
                });
            </script> 
            <div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
                <center>
                    <a href="<?=$sit['url']?>" target="_blank"><img src="<?=(empty($sit['img']) ? 'img/icons/askfm_like.png' : $sit['img'])?>" width="48" height="48" alt="<?=truncate($sit['title'], 10)?>" title="<?=truncate($sit['title'], 60)?>" border="0" /></a><br /><b><?=$lang['b_42']?></b>: <?=($sit['cpc']-1)?><br>
                    <a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=hideref($sit['url'])?>','Ask.fm');" class="followbutton"><?=$lang['askfmlike_05']?></a>
                    ...
                    <span style="position:absolute;bottom:1px;right:2px;"><a href="javascript:void(0);" onclick="report_page('<?=$sit['id']?>','<?=base64_encode($sit['url'])?>','askfm');"><img src="img/report.png" alt="Report" title="Report" border="0" /></a></span>
                </center>
            </div>
            <?}?>
    </div>
