...
if(empty($url) || empty($title)){
	...
	if($db->QueryGetNumRows("SELECT id FROM `twitter_favorite` WHERE `tweet_id`='".$twitter[0]['id']."'") > 0){
		...
		$db->Query("INSERT INTO `twitter_favorite` (user, url, tweet_id, title, max_clicks, daily_clicks, cpc, country, sex) VALUES('".$data['id']."', 'https://twitter.com/".$twitter[0]['user']['screen_name']."/status/".$twitter[0]['id']."', '".$twitter[0]['id']."', '".$title."', '".$max_clicks."', '".$daily_clicks."', '".$cpc."', '".$country."', '".$gender."') ");
