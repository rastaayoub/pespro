...
$sites = $db->QueryFetchArrayAll("SELECT a.id, a.inst_id, a.url, a.title, a.photo, a.cpc, b.premium FROM inst_likes a LEFT JOIN users b ON b.id = a.user LEFT JOIN inst_liked c ON c.user_id = '".$data['id']."' AND c.site_id = a.id WHERE a.active = '0' AND (a.max_clicks > a.clicks OR a.max_clicks = '0') AND (a.daily_clicks > a.today_clicks OR a.daily_clicks = '0') AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site_id IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC".($site['mysql_random'] == 1 ? ', RAND()' : '')." LIMIT 14");
...
<div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
	<center>
		...
		<a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=hideref($sit['url'])?>','Instagram');" class="followbutton"><?=$lang['inst_likes_05']?></a>
		...
		<span style="position:absolute;bottom:1px;right:2px;"><a href="javascript:void(0);" onclick="report_page('<?=$sit['id']?>','<?=base64_encode($sit['url'])?>','inst_likes');"><img src="img/report.png" alt="Report" title="Report" border="0" /></a></span>
	</center>
</div>
<?}?>
</div>
